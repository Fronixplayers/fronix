...
(function() {
  ...
  const html=defaults.map(c=>`<div class="flag-pill"><img src="https://flagcdn.com/48x36/${c.code.toLowerCase()}.png" alt="${c.name}" style="width:28px;height:21px;border-radius:4px;object-fit:cover;" onerror="this.style.display='none'"><span>${c.name}</span></div>`).join('');
  const t1=document.getElementById('flagTrack1');
  const t2=document.getElementById('flagTrack2');
  if(t1&&!t1.children.length){t1.innerHTML=html+html;}
  if(t2&&!t2.children.length){t2.innerHTML=html+html;}
})();
