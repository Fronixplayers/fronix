...
function _renderPopup(p) {
  ...
  // Respect "Show On" rules
  const key = 'popup_seen_' + (p.id || p.title || 'default');
  if (p.showOn === 'once') {
    if (sessionStorage.getItem(key)) return;
  } else if (p.showOn === 'daily') {
    const last = localStorage.getItem(key);
    const today = new Date().toDateString();
    if (last === today) return;
  }

  const delay = (parseInt(p.delay) || 0) * 1000;

  setTimeout(() => {
    const bg = p.bgColor || '#0e0e1f';
    const type = p.type || 'center';

    // Build popup DOM
    const overlay = document.createElement('div');
    overlay.id = 'dsranPopupOverlay';

    let posStyle = '';
    let overlayBg = 'background:rgba(0,0,0,.6);';
    if (type === 'center') {
      posStyle = 'display:flex;align-items:center;justify-content:center;';
    } else if (type === 'bottom-right') {
      posStyle = 'display:flex;align-items:flex-end;justify-content:flex-end;padding:24px;';
      overlayBg = 'background:transparent;pointer-events:none;';
    } else if (type === 'top-banner') {
      posStyle = 'display:flex;align-items:flex-start;justify-content:center;';
      overlayBg = 'background:transparent;pointer-events:none;';
    } else if (type === 'fullscreen') {
      posStyle = 'display:flex;align-items:center;justify-content:center;';
    }

    overlay.style.cssText = `position:fixed;inset:0;z-index:99999;${overlayBg}${posStyle}`;

    const cardMaxWidth = type === 'fullscreen' ? '100%' : type === 'bottom-right' ? '320px' : '420px';
    const cardHeight = type === 'fullscreen' ? '100%' : 'auto';
    const cardRadius = type === 'fullscreen' ? '0' : type === 'top-banner' ? '0 0 16px 16px' : '20px';

    const card = document.createElement('div');
    card.style.cssText = `
      background:${bg};color:#fff;max-width:${cardMaxWidth};width:100%;height:${cardHeight};
      border-radius:${cardRadius};padding:${type==='top-banner'?'16px 24px':'28px'};
      box-shadow:0 25px 60px rgba(0,0,0,.4);position:relative;pointer-events:auto;
      animation:dsranPopupIn .4s cubic-bezier(.4,0,.2,1);
      ${type==='top-banner' ? 'display:flex;align-items:center;gap:16px;flex-wrap:wrap;justify-content:center;' : ''}
    `;

    card.innerHTML = `
      <button id="dsranPopupClose" style="position:absolute;top:12px;right:12px;background:rgba(255,255,255,.15);border:none;width:30px;height:30px;border-radius:50%;color:#fff;cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center;z-index:2;">
        <i class="fas fa-times"></i>
      </button>
      ${p.image ? `<img src="${p.image}" alt="" style="width:100%;border-radius:12px;margin-bottom:16px;max-height:200px;object-fit:cover;">` : ''}
      ${p.title ? `<h3 style="font-size:1.3rem;font-weight:800;margin-bottom:8px;">${p.title}</h3>` : ''}
      ${p.body ? `<p style="color:rgba(255,255,255,.8);font-size:.92rem;line-height:1.6;margin-bottom:${p.btnText?'18px':'0'};">${p.body}</p>` : ''}
      ${p.btnText ? `<a href="${p.btnUrl||'#'}" id="dsranPopupBtn" style="display:inline-block;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;padding:10px 22px;border-radius:10px;text-decoration:none;font-weight:600;font-size:.9rem;">${p.btnText}</a>` : ''}
    `;
    ...
    if (!document.getElementById('dsranPopupStyle')) {
      const style = document.createElement('style');
      style.id = 'dsranPopupStyle';
      style.textContent = `@keyframes dsranPopupIn{from{opacity:0;transform:translateY(20px) scale(.96);}to{opacity:1;transform:translateY(0) scale(1);}}`;
      document.head.appendChild(style);
    }

    function closePopup() {
      overlay.remove();
      if (p.showOn === 'once') sessionStorage.setItem(key, '1');
      if (p.showOn === 'daily') localStorage.setItem(key, new Date().toDateString());
    }

    document.getElementById('dsranPopupClose').addEventListener('click', closePopup);
    overlay.addEventListener('click', e => { if (e.target === overlay && overlayBg.includes('rgba')) closePopup(); });
    const btn = document.getElementById('dsranPopupBtn');
    if (btn) btn.addEventListener('click', () => { if (p.showOn !== 'every') closePopup(); });
...
════════════════════════════════ */
(async () => {
  const [hero,socials,stats,contact,footer,seo,services,projects,certs,press,countries,popups] = await Promise.all([
    _get('hero'),_get('socials'),_get('stats'),_get('contact'),_get('footer'),_get('seo'),
    _list('services'),_list('projects'),_list('certificates'),_list('press'),_list('countries'),_list('popups')
  ]);
  _hero(hero);
  _socials(socials);
  _stats(stats);
  ...
  // Render the first active popup found
  const activePopup = popups.find(p => p.active);
  if (activePopup) _renderPopup(activePopup);
})();
