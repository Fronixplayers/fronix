...
function renderStudentCards(list) {
    ...
    grid.innerHTML = list.map(u => {
        const isVer  = u.isVerified, isPend = u.verificationPending, isRej = u.verificationRejected;
        ...
        let joinedTxt = '—';
        if (u.createdAt) joinedTxt = u.createdAt.toDate().toLocaleDateString('en-IN', { day:'numeric', month:'short', year:'numeric' });

        let lastLoginTxt = '—', lastLoginFull = '';
        if (u.lastLogin) {
            const d2 = u.lastLogin.toDate(), now = new Date();
            const diff = Math.floor((now - d2) / 1000);
            lastLoginTxt = diff < 60 ? 'Just now' : diff < 3600 ? Math.floor(diff/60)+'m ago'
        ...
        let timeSpentTxt = '—';
        if (u.totalTimeSpent > 0) {
            const s = u.totalTimeSpent;
            timeSpentTxt = s < 60 ? s+'s' : s < 3600 ? Math.floor(s/60)+'m '+Math.floor(s%60)+'s'
                         : Math.floor(s/3600)+'h '+Math.floor((s%3600)/60)+'m';
        }

        const avatar = u.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${u.uid}`;
        const blockBtn = u.isBlocked
            ? `<button class="action-btn btn-unblock" onclick="window.toggleBlockUser('${u.uid}',false)" style="flex:1;justify-content:center;"><i class="fas fa-lock-open"></i> Unblock</button>`
            : `<button class="action-btn btn-block"   onclick="window.toggleBlockUser('${u.uid}',true)"  style="flex:1;justify-content:center;"><i class="fas fa-ban"></i> Block</button>`;

        const phoneRow    = u.phone    ? `<div class="sinfo-row"><i class="fas fa-phone"></i><span>${u.phone}</span></div>` : '';
        const collegeRow  = u.college  ? `<div class="sinfo-row"><i class="fas fa-university"></i><span>${u.college}</span></div>` : '';
        const courseRow   = u.course   ? `<div class="sinfo-row"><i class="fas fa-book"></i><span>${u.course}</span></div>` : '';
        const locationRow = u.location ? `<div class="sinfo-row"><i class="fas fa-map-marker-alt"></i><span>${u.location}</span></div>` : '';
        const extraFields = phoneRow + collegeRow + courseRow + locationRow
            || '<div style="color:#ccc;font-size:0.79rem;font-style:italic;">No extra details at signup</div>';

        const blockedBanner = u.isBlocked
            ? `<div style="background:#fee2e2;color:#ef4444;font-size:0.76rem;font-weight:700;padding:4px 10px;border-radius:6px;margin-bottom:10px;text-align:center;">🚫 Account Blocked</div>` : '';

        return `
        <div class="student-card">
            ${blockedBanner}
            <div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:14px;">
                <img src="${avatar}" alt="" onerror="this.src='https://api.dicebear.com/7.x/avataaars/svg?seed=${u.uid}'"
                     style="width:48px;height:48px;border-radius:50%;object-fit:cover;border:2px solid ${verColor};flex-shrink:0;">
                <div style="flex:1;min-width:0;">
                    <div style="font-weight:800;font-size:0.97rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${u.name || 'Unknown'}</div>
                    <div style="color:#888;font-size:0.79rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-bottom:5px;">${u.email || '—'}</div>
                    <span style="background:${verBg};color:${verColor};font-size:0.72rem;font-weight:700;padding:2px 9px;border-radius:20px;">${verTxt}</span>
                </div>
            </div>
            <div style="background:#f8fafc;border-radius:10px;padding:11px 13px;margin-bottom:12px;border:1px solid #e5e7eb;font-size:0.82rem;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px;">
                <div style="background:#f0f9ff;border-radius:8px;padding:9px 11px;border:1px solid #bae6fd;">
                    <div style="font-size:0.68rem;color:#0284c7;font-weight:700;margin-bottom:2px;text-transform:uppercase;letter-spacing:0.04em;">Last Login</div>
                ...
                <div style="background:#f0fdf4;border-radius:8px;padding:9px 11px;border:1px solid #bbf7d0;">
                    <div style="font-size:0.68rem;color:#16a34a;font-weight:700;margin-bottom:2px;text-transform:uppercase;letter-spacing:0.04em;">Time on Site</div>
