// js/admin.js — FronixLearner Admin Dashboard (ALL FIXES APPLIED)
        ...
        snap.forEach(d => {
            ...
            <tr>
                <td><div style="display:flex;align-items:center;gap:10px;">
                    ${vid ? `<img src="https://img.youtube.com/vi/${vid}/default.jpg" style="width:50px;height:35px;border-radius:6px;object-fit:cover;" onerror="this.style.display='none'">` : ''}
                    <strong>${c.title}</strong></div></td>
                <td>${c.instructor}</td>
                <td>${c.category||'General'}</td>
                <td>${badge}</td>
                <td>${c.playlist?c.playlist.length:1}</td>
                <td><button class="action-btn btn-trash" onclick="window.delCourse('${d.id}')"><i class="fas fa-trash"></i></button></td>
            </tr>`;
        });
    });
}
...
// ─── FIX: VERIFICATIONS — Admin can VIEW the KYC image ──
function loadVerifications() {
    onSnapshot(query(collection(db,"users"), where("verificationPending","==",true)), snap => {
        ...
        snap.forEach(d => {
            const u = d.data();
            // FIX: idProofUrl is base64 — show as clickable image preview
            let idHtml = '<span style="color:#aaa;font-size:0.8rem;">No image submitted</span>';
            if (u.idProofUrl && u.idProofUrl.startsWith('data:')) {
                idHtml = `
                <div style="margin-top:6px;">
                    <img src="${u.idProofUrl}"
                         style="width:120px;height:80px;object-fit:cover;border-radius:8px;border:2px solid #e5e7eb;cursor:pointer;"
                         onclick="window.viewKYC('${d.id}')"
...
// View KYC image full size in a lightbox
...
// ─── SUPPORT TICKETS (replaces chatbot) ───────────────
